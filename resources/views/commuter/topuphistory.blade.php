...
<head>
    ...
    <style>
        ...
        .pagination a { color: #666; background: #111; border-color: #1e1e1e; }
        .pagination a:hover { color: #fff; background: #1a1a1a; border-color: #333; }
        .pagination .active { color: #fff; background: #2563eb; border-color: #2563eb; box-shadow: 0 2px 12px rgba(37, 99, 235, 0.25); }
...
<body class="antialiased text-white">
    ...
    <header class="fixed top-4 left-4 right-4 sm:top-5 sm:left-5 sm:right-5 z-50 flex items-center justify-between gap-3 pointer-events-none">
        <div class="glass-panel p-3 sm:p-3.5 rounded-2xl pointer-events-auto flex items-center gap-3 min-w-0">
            ...
            <div class="w-px h-6 bg-[#222] mx-0.5 hidden sm:block"></div>
            <span class="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.15em] text-[#555] hidden sm:inline">Top-up History</span>
        </div>
        <div class="flex items-center gap-2 pointer-events-auto z-50 shrink-0">
            <a href="{{ route('payment.topup') }}" class="header-btn bg-blue-600 px-3 sm:px-4 py-2 rounded-xl text-white text-[9px] sm:text-[10px] font-bold cursor-pointer uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-blue-600/20 hover:bg-blue-500 transition active:scale-[0.98]">
            ...
            <div class="hidden lg:block lg:col-span-4">
                <div class="glass-card p-6 rounded-[1.5rem] sticky top-24">
                    ...
                    <form action="{{ route('payment.topup.history') }}" method="GET" class="space-y-4">
                        ...
                        <!-- Payment Method -->
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-bold uppercase tracking-[0.15em] text-[#444] ml-1">Payment Method</label>
                            <div class="relative">
                                <i class="fa-solid fa-credit-card absolute left-3.5 top-1/2 -translate-y-1/2 text-[10px] text-[#444]"></i>
                                <select name="method"
                                    class="w-full bg-[#111] border border-[#1e1e1e] rounded-xl pl-10 pr-4 py-2.5 text-[12px] text-white outline-none transition appearance-none cursor-pointer">
                                    <option value="" {{ !request('method') ? 'selected' : '' }}>All Methods</option>
                                    <option value="gcash" {{ request('method') === 'gcash' ? 'selected' : '' }}>GCash</option>
                                    <option value="maya" {{ request('method') === 'maya' ? 'selected' : '' }}>Maya</option>
                                    <option value="admin" {{ request('method') === 'admin' ? 'selected' : '' }}>Admin Settlement</option>
                                </select>
                                <i class="fa-solid fa-chevron-down absolute right-3.5 top-1/2 -translate-y-1/2 text-[8px] text-[#444] pointer-events-none"></i>
                            </div>
                        </div>
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                        ...
                        <div class="glass-card border-t-0 rounded-t-none p-4 pt-3">
                            <form action="{{ route('payment.topup.history') }}" method="GET" class="space-y-3">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    @if(request()->hasAny(['from_date', 'to_date', 'search', 'method']))
                                        <a href="{{ route('payment.topup.history') }}" class="px-4 py-2.5 rounded-xl bg-[#111] border border-[#1e1e1e] text-[10px] font-bold text-[#555] hover:text-white hover:border-[#333] transition flex items-center">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-lg bg-[#111] border border-[#1e1e1e] flex items-center justify-center">
                            <i class="fa-solid fa-clock-rotate-left text-[10px] text-[#555]"></i>
                        </div>
                        <span class="text-[9px] font-bold uppercase tracking-[0.15em] text-[#555]">All Top-ups</span>
                    </div>
                    <span class="text-[9px] font-bold text-[#333]">{{ $transactions->total() }} results</span>
                </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-[#1a1a1a]">
                            @forelse($transactions as $tx)
                                <tr class="table-row">
                                    <td class="px-6 py-4">
                                        <p class="text-[11px] font-bold text-[#ccc]">{{ $tx->created_at->format('M d, Y') }}</p>
                                        <p class="text-[9px] text-[#444] mt-0.5">{{ $tx->created_at->format('h:i A') }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="text-[10px] font-bold text-blue-400/80 bg-blue-500/10 border border-blue-500/15 px-2 py-1 rounded-md font-mono">#{{ $tx->id }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2.5">
                                            <div class="w-7 h-7 rounded-lg bg-[#111] border border-[#1e1e1e] flex items-center justify-center shrink-0">
                                                @if($tx->payment_method == 'gcash')
                                                    <i class="fa-solid fa-mobile-screen-button text-[9px] text-blue-400"></i>
                                                @elseif($tx->payment_method == 'maya')
                                                    <i class="fa-solid fa-bolt text-[9px] text-emerald-400"></i>
                                                @else
                                                    <i class="fa-solid fa-user-tie text-[9px] text-amber-400"></i>
                                                @endif
                                            </div>
                                            <span class="text-[11px] font-bold text-[#888] capitalize">{{ $tx->payment_method }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <span class="text-[12px] font-bold text-emerald-400">+₱{{ number_format($tx->amount_added, 2) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="py-16">
                                        <div class="flex flex-col items-center justify-center">
                                            <div class="w-12 h-12 rounded-xl bg-[#111] border border-[#1e1e1e] flex items-center justify-center mb-3">
                                                <i class="fa-solid fa-receipt text-lg text-[#333]"></i>
                                            </div>
                                            <p class="text-[11px] text-[#444] font-medium">No top-up history found</p>
                                            <a href="{{ route('payment.topup') }}" class="text-[9px] text-blue-400 font-bold uppercase tracking-wider mt-2 hover:text-blue-300 transition">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex items-start gap-3 min-w-0 flex-1">
                                    <div class="w-9 h-9 rounded-lg bg-[#111] border border-[#1e1e1e] flex items-center justify-center shrink-0 mt-0.5">
                                        @if($tx->payment_method == 'gcash')
                                            <i class="fa-solid fa-mobile-screen-button text-[10px] text-blue-400"></i>
                                        @elseif($tx->payment_method == 'maya')
                                            <i class="fa-solid fa-bolt text-[10px] text-emerald-400"></i>
                                        @else
                                            <i class="fa-solid fa-user-tie text-[10px] text-amber-400"></i>
                                        @endif
                                    </div>
                                    <div class="min-w-0">
                                        <span class="text-[10px] font-bold text-blue-400/80 bg-blue-500/10 border border-blue-500/15 px-1.5 py-0.5 rounded font-mono">#{{ $tx->id }}</span>
                                        <p class="text-[10px] font-bold text-[#888] capitalize mt-1.5">{{ $tx->payment_method }}</p>
                                        <p class="text-[8px] text-[#444] font-bold uppercase tracking-wider mt-0.5">
                                            {{ $tx->created_at->format('M d, Y') }} • {{ $tx->created_at->format('h:i A') }}
                                        </p>
                                    </div>
                                </div>
                                <div class="text-right shrink-0">
                                    <p class="text-[12px] font-bold text-emerald-400">+₱{{ number_format($tx->amount_added, 2) }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="glass-card p-10 rounded-[1.25rem] text-center">
                            ...
                            <p class="text-[11px] text-[#444] font-medium">No top-up history found</p>
                            <a href="{{ route('payment.topup') }}" class="inline-flex items-center gap-1.5 text-[9px] text-blue-400 font-bold uppercase tracking-wider mt-2 hover:text-blue-300 transition">
                ...
                <!-- Footer -->
                <p class="text-center text-[8px] text-[#222] uppercase tracking-[0.2em] pt-4">
