...
<body x-data="{
    ...
    <main :class="open ? 'ml-72' : 'ml-20'" x-data="{ editing: false, search: '' }" class="sidebar-transition p-8 md:p-12 min-h-screen">
            ...
            <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 shrink-0">
                ...
                <div class="flex gap-3 self-start">
                    ...
                    <input type="text" x-model="search" placeholder="Search logs..."
                        class="bg-[#0a0a0a] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:ring-1 focus:ring-blue-500/50 outline-none w-48">
                </div>
            </header>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 shrink-0">
                ...
                <div class="lg:col-span-2 glass rounded-[2rem] p-6 md:p-8 border border-white/5">
                    ...
                    <div class="border-t border-white/5 pt-5">
                        <h4 class="text-[10px] uppercase font-black text-white/30 tracking-widest mb-3">Cost Per
                            Kilometer Summary</h4>
                ...
                <div class="glass rounded-[2rem] p-6 border border-white/5 flex flex-col gap-6">
                    <a rel="noopener noreferrer" target="_blank"
                        href="https://www.edmunds.com/car-maintenance/guide-page.html"
                        class="flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] hover:border-white/10 transition-all group">
                        <div
                            class="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 group-hover:bg-blue-500/20 transition-colors shrink-0">
                            <i class="fa-solid fa-book-open text-lg"></i>
                        </div>
                        <div class="flex-1 min-w-0">
                            <span
                                class="text-sm font-bold text-white/90 block group-hover:text-white transition-colors">Maintenance
                                Guide</span>
                            <span class="text-xs text-white/40 flex items-center gap-1">View scheduled services <i
                                    class="fa-solid fa-arrow-right text-[10px] group-hover:translate-x-1 transition-transform"></i></span>
                        </div>
                    </a>

                    <button @click="showModal = true"
                        class="w-full py-3.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold shadow-lg shadow-blue-900/40 transition-all flex items-center justify-center gap-2">
                    ...
                    <table class="w-full text-left">
                        <thead class="sticky top-0 z-10">
                            <tr class="border-b border-white/5 bg-[#0a0a0a]">
                                <th
                                    class="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-blue-500 w-[15%]">
                                    Date & Odo</th>
                                <th
                                    class="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-blue-500 w-[30%]">
                                    Service Details</th>
                                <th
                                    class="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-blue-500 w-[25%]">
                                    Cost Breakdown</th>
                                <th
                                    class="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-blue-500 w-[20%]">
                                    Notes</th>
                                <th
                                    class="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-blue-500 w-[10%] text-center">
                                    Actions</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-white/[0.02] transition-colors"
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="text-sm text-white/70 font-medium">
                                            {{ $log->service_date?->format('M d, Y') ?? '—' }}</div>
                                        <div class="text-xs text-white/30 font-mono mt-0.5">
                                            {{ number_format($log->mileage_at_service) }} km</div>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="text-sm text-white font-semibold">
                                            {{ $log->maintenanceTask?->tasks_performed ?? 'Unknown Task' }}</div>
                                        <div class="text-xs text-white/40 mt-0.5">{{ $log->performed_by ?? '—' }}</div>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="text-sm text-white font-bold font-mono">₱
                                            {{ number_format($log->cost, 2) }}</div>
                                        @if ($log->cost == 0)
                                    ...
                                    <td class="px-5 py-4">
                                        @if ($log->remarks && str($log->remarks)->lower()->contains('warranty'))
                                            <span
                                                class="inline-block text-xs text-emerald-400 bg-emerald-500/5 px-2 py-1 rounded border border-emerald-500/10">Under
                                                Warranty</span>
                                        @elseif($log->remarks)
                                            <span
                                                class="text-xs text-white/30">{{ \Illuminate\Support\Str::limit($log->remarks, 40) }}</span>
                                        @else
                                    ...
                                    <td class="px-5 py-4 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            <button @click="openEdit(@js($log->only('id', 'vm_id', 'service_date_formatted', 'mileage_at_service', 'maintenance_task_id', 'performed_by', 'cost', 'invoice_number', 'remarks')))"
                                                class="w-8 h-8 rounded-lg hover:bg-white/[0.06] flex items-center justify-center text-white/30 hover:text-blue-400 transition-colors"
                                                title="Edit">
                                                <i class="fa-solid fa-pen text-xs"></i>
                                            </button>
                                            <form method="POST"
                                                :action="`{{ route('maintenance-manager.vehicle-maintenance-log.destroy', '__ID__') }}`
                                                ...
                                                <button type="submit"
                                                    ...
                                                    class="w-8 h-8 rounded-lg hover:bg-rose-500/[0.08] flex items-center justify-center text-white/30 hover:text-rose-400 transition-colors"
                                                    title="Delete">
        ...
        <div
            class="-mx-8 md:-mx-12 -mb-8 md:-mb-12 mt-4 px-8 md:px-12 py-4 bg-[#0a0a0a] border-t border-white/5 flex justify-between items-center text-xs text-white/30 shrink-0">
            <span>Showing {{ $totalServices }} {{ $totalServices === 1 ? 'entry' : 'entries' }}</span>
            <div class="flex gap-2">
                <button
                    class="px-3 py-1 rounded border border-white/10 hover:bg-white/5 transition opacity-50 cursor-not-allowed"
    ...
    <template x-teleport="body">
        <div x-show="showModal" x-cloak @keydown.escape.window="showModal = false"
            ...
            <form x-show="showModal" method="POST"
                ...
                <div class="flex items-center justify-between px-8 pt-7 pb-5 border-b border-white/5 shrink-0">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-11 h-11 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400">
                ...
                <div class="px-8 pt-5 pb-0 shrink-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Date</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Odometer</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Work
                            Performed</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Performed
                            By</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Cost</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Invoice
                            #</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-blue-400/60 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-md">Remarks</span>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto modal-scroll px-8 py-6 space-y-5">
                    ...
                    @if ($errors->any())
                        <div
                            class="bg-red-500/5 border border-red-500/20 text-red-400 text-xs font-medium px-4 py-3 rounded-xl flex items-start gap-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="service_date"
                                class="col-header block text-[10px] uppercase font-bold text-white/40 tracking-widest mb-2">
                        ...
                        <div>
                            <label for="mileage_at_service"
                                class="col-header block text-[10px] uppercase font-bold text-white/40 tracking-widest mb-2">
                                Odometer (km) <span class="text-red-400/70">*</span>
                            </label>
                            <input id="mileage_at_service" type="number" name="mileage_at_service"
                                value="{{ old('mileage_at_service') }}" min="0" placeholder="e.g. 21000"
                                required
                    ...
                    <div>
                        <label for="maintenance_task_id"
                            class="col-header block text-[10px] uppercase font-bold text-white/40 tracking-widest mb-2">
                            Work Performed <span class="text-red-400/70">*</span>
                        </label>
                        <select id="maintenance_task_id" name="maintenance_task_id" required
                            ...
                            <option value="" disabled {{ !old('maintenance_task_id') ? 'selected' : '' }}>Select
                                a task...</option>
                            @foreach ($maintenanceTasks as $task)
                                <option value="{{ $task->id }}"
                                    {{ old('maintenance_task_id') == $task->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input id="performed_by" type="text" name="performed_by"
                            value="{{ old('performed_by') }}" placeholder="e.g. Castrol Shop, In-House" required
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                <span
                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-xs text-white/20 font-mono">₱</span>
                        ...
                        <div>
                            ...
                            <input id="invoice_number" type="text" name="invoice_number"
                                value="{{ old('invoice_number') }}" placeholder="e.g. INV-0050"
                ...
                <!-- Footer -->
                <div
                    class="flex items-center justify-end gap-3 px-8 py-5 border-t border-white/5 shrink-0 bg-[#0a0a0a]/60">
                    <button type="button" @click="showModal = false"
    ...
    <template x-teleport="body">
        <div x-show="showEditModal" x-cloak @keydown.escape.window="showEditModal = false"
            ...
            <form x-show="showEditModal" method="POST"
                ...
                <div class="flex items-center justify-between px-8 pt-7 pb-5 border-b border-white/5 shrink-0">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-11 h-11 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400">
                ...
                <div class="px-8 pt-5 pb-0 shrink-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Date</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Odometer</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Work
                            Performed</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Performed
                            By</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Cost</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Invoice
                            #</span>
                        <span
                            class="text-[9px] uppercase font-bold tracking-[0.12em] text-amber-400/60 bg-amber-500/5 border border-amber-500/10 px-2.5 py-1 rounded-md">Remarks</span>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto modal-scroll px-8 py-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="service_date" x-model="editLog.service_date" required
                        ...
                        <div>
                            ...
                            <input type="number" name="mileage_at_service" x-model="editLog.mileage_at_service"
                                min="0" required
                    ...
                    <div>
                        ...
                        <select name="maintenance_task_id" x-model="editLog.maintenance_task_id" required
                    ...
                    <div>
                        ...
                        <input type="text" name="performed_by" x-model="editLog.performed_by" required
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                <span
                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-xs text-white/20 font-mono">₱</span>
                                <input type="number" name="cost" x-model="editLog.cost" min="0" required
                        ...
                        <div>
                            ...
                            <input type="text" name="invoice_number" x-model="editLog.invoice_number"
                                class="form-input w-full rounded-xl px-4 py-3 text-sm placeholder:text-white/15 font-mono">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="remarks" x-model="editLog.remarks" rows="3"
