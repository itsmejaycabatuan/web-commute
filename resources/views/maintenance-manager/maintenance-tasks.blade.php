...
<body class="antialiased text-gray-900 dark:text-white bg-white dark:bg-[#050505]" x-data="{
    ...
    <main :class="$store.sidebar.open ? 'md:ml-72' : 'md:ml-20'"
        ...
        @php
            ...
            $withMonths = $tasks
                ->whereNotNull('months_between_service')
                ->where('months_between_service', '>', 0)
                ->count();
        @endphp
        ...
        <div class="lg:hidden mb-5">
            <div class="glass-card p-4 rounded-[1.25rem]">
                ...
                <div class="flex items-center gap-2 pt-3.5 mt-3.5 border-t border-gray-200 dark:border-[#1e1e1e]">
                    ...
                    <span class="font-mono text-[9px] text-gray-400 dark:text-[#444]">Tasks</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-3 sm:gap-4 mb-5 sm:mb-6">
            <div class="glass-card p-4 sm:p-5 rounded-[1.25rem] border-l-2 border-l-blue-500">
                ...
                <div class="flex items-baseline gap-1.5">
                    <span
                        class="text-xl sm:text-2xl font-black tracking-tight text-gray-900 dark:text-white">{{ $totalTasks }}</span>
                    <span
                        class="text-xs font-bold text-gray-400 dark:text-[#555]">{{ $totalTasks === 1 ? 'item' : 'items' }}</span>
                </div>
            </div>
            <div class="glass-card p-4 sm:p-5 rounded-[1.25rem] border-l-2 border-l-emerald-500">
                ...
                <div class="flex items-baseline gap-1.5">
                    <span
                        class="text-xl sm:text-2xl font-black tracking-tight text-emerald-600 dark:text-emerald-400">{{ $withMiles }}</span>
                    <span class="text-xs font-bold text-gray-400 dark:text-[#555]">tasks</span>
                </div>
            </div>
            <div class="glass-card p-4 sm:p-5 rounded-[1.25rem] border-l-2 border-l-purple-500">
                ...
                <div class="flex items-baseline gap-1.5">
                    <span
                        class="text-xl sm:text-2xl font-black tracking-tight text-purple-600 dark:text-purple-400">{{ $withMonths }}</span>
                    <span class="text-xs font-bold text-gray-400 dark:text-[#555]">tasks</span>
                </div>
            </div>
        </div>
        ...
        <div class="glass-card rounded-[1.25rem] sm:rounded-[1.5rem] overflow-hidden">
            <div class="p-4 sm:p-6 pb-0">
                <div class="flex items-center justify-between mb-4 sm:mb-5">
                    ...
                    <span
                        class="text-[7px] sm:text-[8px] font-bold text-gray-400 dark:text-[#333] uppercase tracking-widest">{{ $totalTasks }}
                        tasks</span>
                </div>
            </div>
                            ...
                            <tr class="table-row group">
                                <td class="px-4 sm:px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            <i
                                                class="fa-solid fa-wrench text-[8px] text-blue-500 dark:text-blue-400"></i>
                                        </div>
                                        <p
                                            class="text-[10px] sm:text-[11px] font-bold text-gray-700 dark:text-[#ccc] truncate max-w-[280px]">
                                ...
                                <td class="px-4 sm:px-6 py-3.5 text-right">
                                    @if ($task->miles_between_service)
                                        <span
                                            class="text-[10px] sm:text-[11px] font-bold text-gray-500 dark:text-[#888] font-mono">{{ number_format($task->miles_between_service) }}
                                            km</span>
                                    @else
                                        <span class="text-[10px] sm:text-[11px] text-gray-200 dark:text-[#222]">—</span>
                                    @endif
                                </td>
                                <td class="px-4 sm:px-6 py-3.5 text-right">
                                    @if ($task->months_between_service)
                                        <span
                                            class="text-[10px] sm:text-[11px] font-bold text-gray-500 dark:text-[#888] font-mono">{{ $task->months_between_service }}
                                            mo</span>
                                    @else
                                        <span class="text-[10px] sm:text-[11px] text-gray-200 dark:text-[#222]">—</span>
                                    @endif
                                ...
                                <td colspan="4" class="py-10 sm:py-12">
                                    <div class="flex flex-col items-center justify-center">
                                        <div
                                            ...
                                            <i class="fa-solid fa-inbox text-sm text-gray-300 dark:text-[#333]"></i>
                                        </div>
                                        <p
                                            class="text-[10px] sm:text-[11px] text-gray-400 dark:text-[#444] font-medium">
                                            No tasks defined yet</p>
                                        <p class="text-[8px] text-gray-300 dark:text-[#333] mt-0.5">Click "Add Task" to
                                            create a service interval</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                ...
                <div
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span
                                class="text-[7px] font-bold text-gray-400 dark:text-[#444] uppercase">{{ $withMiles }}
                                km-based</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <div class="w-2 h-2 rounded-sm bg-purple-500"></div>
                            <span
                                class="text-[7px] font-bold text-gray-400 dark:text-[#444] uppercase">{{ $withMonths }}
                                time-based</span>
                        </div>
                    </div>
                    <span class="text-[7px] text-gray-400 dark:text-[#333] font-bold uppercase tracking-widest">Total:
                ...
                <form
                    ...
                    <div
                        ...
                        <p class="text-[9px] text-gray-500 dark:text-[#444] leading-relaxed">
                            ...
                            or <span class="text-gray-700 dark:text-[#666] font-bold">Month Interval</span> must be
                            provided.
                        </p>
                    </div>
