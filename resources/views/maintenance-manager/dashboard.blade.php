    ...
    <main :class="open ? 'ml-72' : 'ml-20'"
        ...
        <div class="max-w-[1600px] mx-auto">
            ...
            <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 class="text-3xl font-black tracking-tight mb-1">Maintenance <span class="text-blue-500">Dashboard</span></h2>
                    <p class="text-white/40 text-sm">Overview of fleet maintenance history, costs, and schedules.</p>
                </div>
                            ...
                            <table class="w-full text-left whitespace-nowrap">
                                <thead>
                                    <tr class="border-b border-white/5 bg-[#0a0a0a]">
                                        ...
                                        @foreach(range(1, 12) as $m)<th class="px-4 py-3 text-[10px] font-bold text-white/30 text-right w-20">{{ ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][$m - 1] }}</th>@endforeach
                                ...
                                <tbody class="divide-y divide-white/5 text-sm">
                                    ...
                                    <tr class="hover:bg-white/[0.02]">
                                        ...
                                        @foreach(range(1, 12) as $m)<td class="px-4 py-2.5 font-mono text-right @if($monthlyStartOdo[$m] !== null) text-white/40 @else text-white/15 @endif">@if($monthlyStartOdo[$m] !== null){{ number_format($monthlyStartOdo[$m]) }}@else—@endif</td>@endforeach
                                    ...
                                    <tr class="hover:bg-white/[0.02]">
                                        ...
                                        @foreach(range(1, 12) as $m)<td class="px-4 py-2.5 font-mono text-right @if($monthlyEndOdo[$m] !== null) text-white/50 @else text-white/15 @endif">@if($monthlyEndOdo[$m] !== null){{ number_format($monthlyEndOdo[$m]) }}@else—@endif</td>@endforeach
                                        <td class="px-4 py-2.5 font-mono text-right text-white/50 border-l border-white/10">
                                    ...
                                    <tr class="hover:bg-white/[0.02]">
                                        ...
                                        @foreach(range(1, 12) as $m)<td class="px-4 py-2.5 font-mono text-right @if($monthlyKm[$m] > 0) text-white/60 @else text-white/15 @endif">@if($monthlyKm[$m] > 0){{ number_format($monthlyKm[$m]) }}@else—@endif</td>@endforeach
                                    ...
                                    <tr class="hover:bg-white/[0.02]">
                                        ...
                                        @foreach(range(1, 12) as $m)<td class="px-4 py-2.5 font-mono text-right @if($monthlyCpk[$m] !== null) text-emerald-400/80 @else text-white/15 @endif">@if($monthlyCpk[$m] !== null)₱{{ number_format($monthlyCpk[$m], 2) }}@else—@endif</td>@endforeach
                                    ...
                                    <tr class="bg-blue-500/[0.03] border-t-2 border-blue-500/20">
                                        ...
                                        @foreach(range(1, 12) as $m)<td class="px-4 py-3.5 font-mono text-right font-bold @if($monthlyTotals[$m] > 0) text-blue-400 @else text-white/20 @endif">₱{{ number_format($monthlyTotals[$m], 2) }}</td>@endforeach
                                ...
                                <tbody class="divide-y divide-white/5 text-sm">
                                    @forelse($costSummary as $taskName => $months)
                                        <tr class="hover:bg-white/[0.02]">
                                            ...
                                            @foreach(range(1, 12) as $m)<td class="px-4 py-2.5 font-mono text-right @if($months[$m] > 0) text-white/50 @else text-white/15 @endif">@if($months[$m] > 0)₱{{ number_format($months[$m], 2) }}@else—@endif</td>@endforeach
                                        </tr>
                                    @empty
                                        <tr>
                            ...
                            <div class="px-6 py-5 border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                                <div class="flex gap-5">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-xs">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <span class="text-white/40 truncate block max-w-[240px]" title="{{ $log['remarks'] ?? '' }}">{{ $log['remarks'] ?? '—' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
