        ...
        <div class="max-w-[1600px] mx-auto">
            ...
            @if (!$fleet)
                <div class="flex flex-col items-center justify-center py-32">
                    <div
                        class="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center text-white/15 mb-4">
            ...
            @else
                ...
                <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h2 class="text-3xl font-black tracking-tight mb-1">Fleet Maintenance <span
                                class="text-blue-500">Summary</span></h2>
                        ...
                        <select id="fleet-selector"
                            ...
                            @foreach ($fleets as $f)
                                <option value="{{ $f->id }}" {{ $fleet->id === $f->id ? 'selected' : '' }}>
                                    {{ $f->vehicle?->plate_number }} — {{ $f->vehicle?->brand }}
                                    {{ $f->vehicle?->model }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="glass rounded-2xl p-6 md:p-8 border border-white/5 mb-6">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center gap-4">
                            <div>
                                <h3 class="text-lg font-bold tracking-tight">{{ $fleet->vehicle?->year }}
                                    {{ $fleet->vehicle?->brand }} {{ $fleet->vehicle?->model }}</h3>
                                <div class="flex items-center gap-3 mt-1">
                                    <span
                                        class="font-mono text-xs text-white/50 bg-white/5 px-2 py-0.5 rounded">{{ $fleet->vehicle?->plate_number }}</span>
                                    @if ($fleet->vehicle?->status === 'active')
                                        <span
                                            ...
                                            <span
                                                class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>Active
                                        </span>
                                    @elseif($fleet->vehicle?->status === 'maintenance')
                                        <span
                                            class="inline-flex items-center gap-1.5 text-[10px] font-bold text-amber-400 bg-amber-500/10 px-2.5 py-0.5 rounded-full border border-amber-500/20">
                                            <span
                                                class="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>Maintenance
                                        </span>
                                    @elseif($fleet->vehicle?->status === 'inactive')
                                        <span
                                            class="inline-flex items-center gap-1.5 text-[10px] font-bold text-rose-400/70 bg-rose-500/[0.06] px-2.5 py-0.5 rounded-full border border-rose-500/10">
                                            <span class="w-1.5 h-1.5 rounded-full bg-rose-400/70"></span>Inactive
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 text-[10px] font-bold text-white/20 bg-white/[0.03] px-2.5 py-0.5 rounded-full border border-white/[0.06]">
                                            <span
                                                class="w-1.5 h-1.5 rounded-full bg-white/15"></span>{{ ucfirst($fleet->vehicle?->status ?? '') }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div
                        class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-6 gap-y-4 text-sm border-t border-white/5 pt-5">
                        <div>
                            <span class="text-[10px] uppercase font-bold text-white/25 block mb-1">Fuel Type</span>
                            <span
                                class="text-white/70 font-medium text-sm">{{ $fleet->vehicle?->fuel_type ?? '—' }}</span>
                        </div>
                        <div>
                            <span class="text-[10px] uppercase font-bold text-white/25 block mb-1">Tank Capacity</span>
                            <span
                                class="text-white/70 font-medium text-sm">{{ $fleet->vehicle?->tank_capacity ? $fleet->vehicle->tank_capacity . ' L' : '—' }}</span>
                        </div>
                        <div>
                            <span class="text-[10px] uppercase font-bold text-white/25 block mb-1">Driver</span>
                            <span
                                class="text-white/70 font-medium text-sm">{{ $fleet->vehicle?->driver?->name ?? 'Unassigned' }}</span>
                        </div>
                        <div>
                            <span class="text-[10px] uppercase font-bold text-white/25 block mb-1">Location</span>
                            <span
                                class="text-white/70 font-medium text-sm">{{ $fleet->vehicle?->location ?? '—' }}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span
                                class="text-white/50 text-xs">{{ $fleet->vehicle?->exp_disposal_date ? $fleet->vehicle->exp_disposal_date->format('M d, Y') : '—' }}</span>
                        </div>
                    </div>

                    <div
                        class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mt-4 pt-4 border-t border-white/5">
                        <div>
                        ...
                        <span class="text-white/30 text-xs">Last updated: <span
                                class="text-white/50">{{ $fleet->vehicle?->updated_at?->format('M d, Y') }}</span></span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    ...
                    <div class="glass rounded-2xl p-5 border border-white/5">
                        ...
                        <p class="text-3xl font-black tracking-tight text-emerald-400">
                            ₱{{ number_format($costPerKm, 2) }}</p>
                    ...
                    <div class="glass rounded-2xl p-5 border border-white/5">
                        ...
                        <p class="text-3xl font-black tracking-tight text-blue-400">
                            ₱{{ number_format($totalServiceCost, 2) }}</p>
                ...
                <div>
                    ...
                    <div x-show="activeTab === 'cpk'" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="glass rounded-2xl border border-white/5 overflow-hidden flex flex-col">
                            <div
                                ...
                                <h3 class="font-bold text-white text-sm">Cost Per Kilometer Summary —
                                    {{ $year }}</h3>
                                <span class="text-[10px] text-white/20 uppercase tracking-wider font-bold">Scroll to
                                    view months <i class="fa-solid fa-arrow-right text-[8px] ml-1"></i></span>
                            </div>
                                ...
                                <table class="w-full text-left whitespace-nowrap">
                                    <thead>
                                        <tr class="border-b border-white/5 bg-[#0a0a0a]">
                                            <th
                                                class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-blue-500 sticky left-0 bg-[#0a0a0a] z-10 text-left min-w-[180px]">
                                                Metric</th>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <th
                                                    class="px-4 py-3 text-[10px] font-bold text-white/30 text-right w-20">
                                                    {{ ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][$m - 1] }}
                                                </th>
                                            @endfor
                                            <th
                                                class="px-4 py-3 text-[10px] font-bold text-blue-400 text-right w-24 border-l border-white/10">
                                                YTD</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-white/5 text-sm">
                                        ...
                                        <tr class="hover:bg-white/[0.02]">
                                            <td class="px-4 py-2.5 text-white/40 sticky left-0 bg-[#050505]">
                                                <i
                                                    class="fa-solid fa-flag-checkered text-white/20 text-[10px] mr-2 w-4 text-center"></i>Starting
                                                Odometer
                                            </td>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <td
                                                    class="px-4 py-2.5 font-mono text-right @if ($monthlyStartOdo[$m] !== null) text-white/40 @else text-white/15 @endif">
                                            ...
                                            @endfor
                                            <td
                                                class="px-4 py-2.5 font-mono text-right text-white/50 border-l border-white/10">
                                        ...
                                        <tr class="hover:bg-white/[0.02]">
                                            <td class="px-4 py-2.5 text-white/40 sticky left-0 bg-[#050505]">
                                                <i
                                                    class="fa-solid fa-flag text-white/20 text-[10px] mr-2 w-4 text-center"></i>Ending
                                                Odometer
                                            </td>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <td
                                                    class="px-4 py-2.5 font-mono text-right @if ($monthlyEndOdo[$m] !== null) text-white/50 @else text-white/15 @endif">
                                        ...
                                        <tr class="hover:bg-white/[0.02]">
                                            <td class="px-4 py-2.5 text-white/60 sticky left-0 bg-[#050505]">
                                                <i
                                                    class="fa-solid fa-road text-white/20 text-[10px] mr-2 w-4 text-center"></i>Monthly
                                                Km
                                            </td>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <td
                                                    class="px-4 py-2.5 font-mono text-right @if ($monthlyKm[$m] > 0) text-white/60 @else text-white/15 @endif">
                                                    @if ($monthlyKm[$m] > 0)
                                            ...
                                            @endfor>
                                            <td
                                                class="px-4 py-2.5 font-mono text-right font-bold text-white/70 border-l border-white/10">
                                        ...
                                        <tr class="hover:bg-white/[0.02]">
                                            <td class="px-4 py-2.5 text-white/60 sticky left-0 bg-[#050505]">
                                                <i
                                                    class="fa-solid fa-gauge text-emerald-400/50 text-[10px] mr-2 w-4 text-center"></i>Service
                                                Cost / Km
                                            </td>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <td
                                                    class="px-4 py-2.5 font-mono text-right @if ($monthlyCpk[$m] !== null) text-emerald-400/80 @else text-white/15 @endif">
                                            ...
                                            @endfor>
                                            <td
                                                class="px-4 py-2.5 font-mono text-right font-bold text-emerald-400 border-l border-white/10">
                                        ...
                                        <tr class="bg-blue-500/[0.03] border-t-2 border-blue-500/20">
                                            ...
                                            @for ($m = 1; $m <= 12; $m++)
                                                <td
                                                    class="px-4 py-3.5 font-mono text-right font-bold @if ($monthlyTotals[$m] > 0) text-blue-400 @else text-white/20 @endif">
                                                    ₱{{ number_format($monthlyTotals[$m], 2) }}
                                                </td>
                                            @endfor
                                            <td
                                                class="px-4 py-3.5 font-mono text-right font-bold text-blue-400 border-l border-white/10">
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-4">
                                    <span class="text-white/25 uppercase font-bold tracking-wider">Annual
                                        Kilometers</span>
                                    <span
                                        class="text-white font-bold font-mono text-sm">{{ number_format($annualKm) }}
                                        km</span>
                                </div>
                                <div class="flex items-center gap-4">
                                    <span class="text-white/25 uppercase font-bold tracking-wider">Avg Cost / Km</span>
                                    <span
                                        class="text-emerald-400 font-bold font-mono text-sm">₱{{ number_format($costPerKm, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'cost'" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="glass rounded-2xl border border-white/5 overflow-hidden flex flex-col">
                            <div
                                class="px-6 py-3.5 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                                <h3 class="font-bold text-white text-sm">Maintenance Cost Summary —
                                    {{ $year }}</h3>
                                <span class="text-[10px] text-white/20 uppercase tracking-wider font-bold">Scroll to
                                    view months <i class="fa-solid fa-arrow-right text-[8px] ml-1"></i></span>
                            </div>
                            <div class="overflow-x-auto table-scroll flex-1">
                                <table class="w-full text-left whitespace-nowrap">
                                    <thead>
                                        <tr class="border-b border-white/5 bg-[#0a0a0a]">
                                            <th
                                                class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-blue-500 sticky left-0 bg-[#0a0a0a] z-10 text-left min-w-[200px]">
                                                Category / Item</th>
                                            @for ($m = 1; $m <= 12; $m++)
                                                <th
                                                    class="px-4 py-3 text-[10px] font-bold text-white/30 text-right w-20">
                                    ...
                                    <tbody class="divide-y divide-white/5 text-sm">
                                        @forelse($costSummary as $taskName => $months)
                                            <tr class="hover:bg-white/[0.02]">
                                                ...
                                                @for ($m = 1; $m <= 12; $m++)
                                                    <td
                                                        class="px-4 py-2.5 font-mono text-right @if ($months[$m] > 0) text-white/50 @else text-white/15 @endif">
                                                        @if ($months[$m] > 0)
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="13"
                                                    class="px-4 py-12 text-center text-white/25 text-sm">No maintenance
                                                    records for {{ $year }}.</td>
                                            </tr>
                                        @endforelse
                            ...
                            <div
                                ...
                                <span
                                    class="text-white font-bold font-mono text-sm">₱{{ number_format($ytdTotal, 2) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'schedule'" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="glass rounded-2xl border border-white/5 overflow-hidden">
                            <div
                                ...
                                <span class="text-[10px] text-white/20 uppercase tracking-wider font-bold">Based on
                                    kilometer intervals</span>
                            </div>
                                        ...
                                        <div class="flex-1 pb-2">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                                <div
                                                    ...
                                                    <i
                                                        class="fa-solid fa-rotate text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Tire Rotation</span></div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-oil-can text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Oil & Filter Change</span></div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-wind text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Air Filter Inspection</span>
                                                </div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-droplet text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Fluid Level Check</span></div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="flex-1 pb-2">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-circle-dot text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Tire Replacement</span></div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-oil-can text-white/25 text-[10px] w-4 text-center"></i><span
                                                ...
                                                <div
                                                    ...
                                                    <i
                                                        class="fa-solid fa-wind text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Air Filter Replacement</span>
                                                </div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-droplet text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Coolant Flush</span></div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="flex-1 pb-2">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                                ...
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-gears text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Transmission Fluid</span></div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-car text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Brake Inspection</span></div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                                ...
                                                <div
                                                    ...
                                                    <i
                                                        class="fa-solid fa-car text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Brake Pad Replacement</span>
                                                </div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-bolt text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Spark Plug Replacement</span>
                                                </div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-gears text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Transmission Service</span></div>
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-droplet text-white/25 text-[10px] w-4 text-center"></i><span
                                                ...
                                                <div
                                                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/5">
                                                    <i
                                                        class="fa-solid fa-magnifying-glass text-white/25 text-[10px] w-4 text-center"></i><span
                                                        class="text-xs text-white/60">Full Inspection</span></div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div x-show="activeTab === 'log'" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="glass rounded-2xl border border-white/5 overflow-hidden">
                            <div
                                ...
                                <span
                                    class="text-[10px] text-white/20 uppercase tracking-wider font-bold">{{ $allLogs->count() }}
                            ...
                            <div class="max-h-[600px] overflow-y-auto log-scroll">
                                ...
                                @forelse($allLogs as $log)
                                    <div
                                        class="px-6 py-5 border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                                        <div class="flex gap-5">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div
                                                    ...
                                                    <span
                                                        class="text-sm font-bold text-white">{{ $log['task_name'] }}</span>
                                                    <span
                                                        class="text-[10px] text-white/30 font-mono shrink-0">{{ $log['service_date'] ?? '—' }}</span>
                                                </div>
                                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-2 text-xs">
                                                    <div>
                                                        <span
                                                            class="text-white/25 uppercase font-bold text-[10px] block">Odometer</span>
                                                        <span
                                                            class="text-white/60 font-mono">{{ number_format($log['mileage']) }}
                                                            km</span>
                                                    </div>
                                                    <div>
                                                        <span
                                                            class="text-white/25 uppercase font-bold text-[10px] block">Cost</span>
                                                        <span
                                                            class="text-white/60 font-mono">₱{{ number_format($log['cost'], 2) }}</span>
                                                    </div>
                                                    <div>
                                                        <span
                                                            class="text-white/25 uppercase font-bold text-[10px] block">Service
                                                            Provider</span>
                                                        <span
                                                            class="text-white/60">{{ $log['performed_by'] ?? '—' }}</span>
                                                    </div>
                                                    <div>
                                                        <span
                                                            class="text-white/25 uppercase font-bold text-[10px] block">Remarks</span>
                                                        <span class="text-white/40 truncate block max-w-[200px]"
                                                ...
                                                @if ($log['invoice_number'])
                                                    <div class="mt-2">
                                                        <span
                                                            class="text-white/25 uppercase font-bold text-[10px]">Invoice:</span>
                                                        <span
                                                            class="text-white/40 font-mono text-[11px] ml-1.5">{{ $log['invoice_number'] }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                @empty
                                    <div class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            <div
                                                class="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center text-white/15">
