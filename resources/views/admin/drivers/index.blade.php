...
<body
        ...
        <div class="max-w-7xl">
            <header class="flex flex-col gap-4 mb-10 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="text-gray-500 text-sm">Manage drivers and approvals. Open the license in a new tab or download the file.</p>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse ($drivers as $driver)
                                ...
                                <tr class="hover:bg-white/[0.02]">
                                            ...
                                            <div class="flex flex-col gap-2 items-start max-w-[140px]">
                                                <a href="{{ $licenseRoute }}" target="_blank" rel="noopener"
                                                    ...
                                                    <img src="{{ $licenseRoute }}" alt="License preview" loading="lazy"
                                                        class="max-h-20 w-full object-contain object-center"
                                                        width="120" height="80">
                                                </a>
                                                <a href="{{ $licenseRoute }}" target="_blank" rel="noopener"
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-gray-500/15 text-gray-400 border border-gray-500/25">{{ $driver->driver_approval_status ? ucfirst($driver->driver_approval_status) : 'Unset' }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 text-gray-500 whitespace-nowrap">{{ $driver->created_at->format('M j, Y') }}</td>
                                    <td class="px-4 py-4 text-right">
                                        <div class="flex flex-wrap gap-1.5 justify-end items-center">
                                            <form action="{{ route('admin.drivers.approve', $driver) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    @if ($driver->driver_approval_status === 'approved') disabled @endif
                                            ...
                                            <form action="{{ route('admin.drivers.unapprove', $driver) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    @if ($driver->driver_approval_status !== 'approved') disabled @endif
                                                    class="px-2.5 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider bg-amber-600/90 hover:bg-amber-500 text-white transition disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:bg-amber-600/90">
                                                    Unapprove
                                                </button>
                                            </form>
                                            <button type="button"
                                                @if ($driver->driver_approval_status === 'rejected') disabled @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">No drivers registered yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div x-show="rejectModal" x-cloak
        ...
        <div class="glass w-full max-w-md p-8 rounded-[2rem] border border-white/10 shadow-2xl text-center" x-on:click.stop>
            <div class="flex justify-center mb-4">
                <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-red-500/15 text-red-400 border border-red-500/25">
            ...
            <p class="text-xs text-gray-500 leading-relaxed mb-8">They will not be able to log in until you approve them again from this list.</p>
