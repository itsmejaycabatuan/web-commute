...
<body class="antialiased text-gray-900 dark:text-white" x-data>
    <div x-data="{
        ...
        init() {
            ...
            @if ($errors->any() && old('_form_type') === 'create') this.showAddModal = true; @endif
            @if ($errors->any() && old('_form_type') === 'edit') this.selectedUser = {
                    id: {{ old('_edit_id', 0) }},
                    email: '{{ old('email', '') }}',
                    email_verified_at: {{ old('mark_verified') ? '"yes"' : 'null' }},
                    created_at: ''
                };
                this.showEditModal = true; @endif
        },
        ...
        <main :class="$store.sidebar.open ? 'md:ml-72' : 'md:ml-20'"
            ...
            <div class="hidden lg:block mb-8">
                ...
                <h1 class="text-2xl sm:text-3xl font-black tracking-tight text-gray-900 dark:text-white">PUJ Commuters
                </h1>
                <p class="text-[11px] text-gray-500 dark:text-[#555] mt-1 flex items-center gap-2">
                    ...
                    <span
                        class="text-gray-700 dark:text-[#888] font-bold">{{ $commuters instanceof \Illuminate\Pagination\LengthAwarePaginator ? $commuters->total() : count($commuters) }}</span>
                                    ...
                                    <td class="px-4 sm:px-6 py-3">
                                        <div class="flex items-center gap-3">
                                            <div
                                                ...
                                                <span class="text-[9px] font-black text-gray-500 dark:text-[#555]"
                                                    x-text="commuter.email.charAt(0).toUpperCase()"></span>
                                            </div>
                                            <span
                                                class="text-[10px] sm:text-[11px] font-bold text-gray-700 dark:text-[#ccc] truncate max-w-[220px]"
                                                x-text="commuter.email"></span>
                                        </div>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3">
                                    ...
                                    <td colspan="5" class="py-12 sm:py-16">
                                        <div class="flex flex-col items-center justify-center">
                                            <div
                                                ...
                                                <i
                                                    class="fa-solid fa-users text-base text-gray-300 dark:text-[#222]"></i>
                                            </div>
                                            <p class="text-[11px] text-gray-400 dark:text-[#444] font-medium mb-4">No
                                                commuter accounts yet</p>
                                            <button @click="showAddModal = true"
        ...
        <div x-show="showAddModal" x-cloak
            ...
            <div @click.away="showAddModal = false"
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white">Add Commuter</h3>
                        <p class="text-[10px] text-gray-500 dark:text-[#555] mt-0.5">Create a new PUJ passenger account
                        </p>
                    </div>
                ...
                <form action="{{ route('commuters.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block mb-1.5 text-[8px] font-bold uppercase tracking-[0.15em] text-gray-400 dark:text-[#444]">Email</label>
                    ...
                    <div>
                        <label
                            class="block mb-1.5 text-[8px] font-bold uppercase tracking-[0.15em] text-gray-400 dark:text-[#444]">Password</label>
                    ...
                    <div class="pt-1 border-t border-gray-200 dark:border-[#1e1e1e]">
                        <label class="flex items-center gap-3 cursor-pointer group py-1">
                            <div class="relative flex items-center justify-center">
                                <input type="checkbox" name="mark_verified" value="1" class="peer sr-only"
                                    {{ old('mark_verified') ? 'checked' : '' }}>
                                <div
                                    class="w-9 h-5 rounded-full bg-gray-200 dark:bg-[#222] border border-gray-300 dark:border-[#2a2a2a] peer-checked:bg-blue-600 peer-checked:border-blue-500 transition">
                                </div>
                                <div
                                    class="absolute left-0.5 w-4 h-4 bg-gray-400 dark:bg-[#555] rounded-full peer-checked:translate-x-4 peer-checked:bg-white transition">
                                </div>
                            </div>
                            <span
                                class="text-[10px] font-bold text-gray-500 dark:text-[#555] group-hover:text-gray-700 dark:group-hover:text-[#888] transition">Mark
                                email as verified</span>
                        </label>
                    </div>
        ...
        <div x-show="showEditModal" x-cloak
            ...
            <div @click.away="showEditModal = false"
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white">Edit Commuter</h3>
                        <p class="text-[10px] text-gray-500 dark:text-[#555] mt-0.5 truncate max-w-[260px]"
                            x-text="selectedUser?.email"></p>
                    </div>
                ...
                <form method="POST" :action="'/commuters/' + (selectedUser?.id || '')" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block mb-1.5 text-[8px] font-bold uppercase tracking-[0.15em] text-gray-400 dark:text-[#444]">New
                            Password</label>
                    ...
                    <div class="pt-1 border-t border-gray-200 dark:border-[#1e1e1e]">
                        <label class="flex items-center gap-3 cursor-pointer group py-1">
                            ...
                            <span
                                class="text-[10px] font-bold text-gray-500 dark:text-[#555] group-hover:text-gray-700 dark:group-hover:text-[#888] transition">Email
                                verified</span>
                        </label>
                    </div>
        ...
        <div x-show="showDeleteModal" x-cloak
            ...
            <div @click.away="showDeleteModal = false" class="glass-panel p-6 sm:p-8 rounded-[2rem] max-w-sm w-full">

                <div class="text-center">
                    <div
                        class="w-14 h-14 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-5 border border-red-500/20">
                        <i class="fa-solid fa-trash text-red-500 dark:text-red-400 text-lg"></i>
                    </div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1.5">Delete Commuter?</h3>
                    <p class="text-[11px] text-gray-500 dark:text-[#555] mb-1">This action is permanent and cannot be
                        undone.</p>
                    <div
