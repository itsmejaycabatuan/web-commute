...
<body class="antialiased text-gray-900 dark:text-white" x-data>
    ...
    <div x-data="{ showLogoutModal: false }" @keydown.escape.window="showLogoutModal = false">
                                    ...
                                    <td colspan="7" class="py-12 sm:py-16">
                                        <div class="flex flex-col items-center justify-center">
                                            <div
                                                ...
                                                <i class="fa-solid fa-list-check text-base text-[#222]"></i></div>
                ...
                <form method="POST" id="violationForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-[120px_1fr] gap-4">
                        <div>
                            ...
                            <input type="text" name="code" x-model="form.code" maxlength="10"
                                class="w-full px-4 py-3 rounded-xl bg-[#111] border border-[#1e1e1e] text-[11px] font-mono font-bold focus:outline-none focus:border-[#333] transition"
                                placeholder="UV06">
                            @error('code')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div><span class="block text-[8px] font-bold text-[#333] mb-1.5">1st Offense</span><input
                                    type="number" name="first" x-model="form.first" min="0"
                                    step="100"
                                    class="w-full px-4 py-3 rounded-xl bg-[#111] border border-[#1e1e1e] text-[11px] font-mono text-right focus:outline-none focus:border-[#333] transition"
                                    placeholder="1,000"></div>
                            <div><span class="block text-[8px] font-bold text-[#333] mb-1.5">2nd Offense</span><input
                                    type="number" name="second" x-model="form.second" min="0"
                                    step="100"
                                    class="w-full px-4 py-3 rounded-xl bg-[#111] border border-[#1e1e1e] text-[11px] font-mono text-right focus:outline-none focus:border-[#333] transition"
                                    placeholder="2,000"></div>
                            <div><span class="block text-[8px] font-bold text-[#333] mb-1.5">3rd Offense</span><input
                                    type="number" name="third" x-model="form.third" min="0"
                                    step="100"
                                    class="w-full px-4 py-3 rounded-xl bg-[#111] border border-[#1e1e1e] text-[11px] font-mono text-right focus:outline-none focus:border-[#333] transition"
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-[8px] font-bold uppercase tracking-[0.15em] text-[#444]">4th+ Offense
                                Penalty</label>
                            <div class="flex items-center gap-2.5">
                                ...
                                <div class="relative flex items-center justify-center w-9 h-5 rounded-full cursor-pointer transition"
                                    ...
                                    <div class="absolute left-0.5 w-4 h-4 rounded-full transition"
                                        :class="form.is_revocation ? 'translate-x-4 bg-white' : 'translate-x-0 bg-[#555]'">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="form.is_revocation" x-transition
                            ...
                            <span class="text-[10px] text-red-400/80 font-medium">Driver's license will be revoked on
                                the 4th offense</span>
                        </div>
                    </div>
        ...
        <div x-show="showDeleteModal" x-cloak
            ...
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0">
            <div @click.stop class="glass-panel p-6 sm:p-8 rounded-[2rem] max-w-sm w-full" x-show="showDeleteModal"
                ...
                <div class="text-center">
                    <div
                        ...
                        <i class="fa-solid fa-trash text-red-400 text-lg"></i></div>
                    ...
                    <div
                        class="inline-flex items-start gap-3 p-4 rounded-xl bg-[#0a0a0a] border border-[#1e1e1e] mb-7 text-left">
            ...
            <div @click.stop class="glass-panel rounded-[2rem] max-w-5xl w-full overflow-hidden flex flex-col"
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div
                                ...
                                <i class="fa-solid fa-layer-group text-[10px] text-emerald-400"></i></div>
                            <div>
                                <h3 class="text-base sm:text-lg font-bold">Bulk Add Violation Codes</h3>
                                <p class="text-[10px] text-[#555] mt-0.5">Add multiple codes at once. Only rows with
                                    Code and Name will be created.</p>
                            </div>
                        </div>
                        ...
                        <template x-for="(row, idx) in bulkRows" :key="idx">
                            <div class="grid gap-1.5 items-center rounded-lg px-1.5 py-1.5 bg-[#111] border border-[#1e1e1e] hover:bg-[#151515] transition"
                                ...
                                <input type="text" x-model="row.code" maxlength="10"
                                    class="w-full px-2.5 py-2 rounded-lg bg-[#0a0a0a] border border-[#1e1e1e] text-[9px] font-mono font-bold focus:outline-none focus:border-[#333] transition"
                                ...
                                <input type="text" x-model="row.name"
                                    class="w-full px-2.5 py-2 rounded-lg bg-[#0a0a0a] border border-[#1e1e1e] text-[9px] font-semibold focus:outline-none focus:border-[#333] transition"
                                ...
                                <div class="flex justify-center">
                                    <button type="button" @click="row.is_revocation = !row.is_revocation"
                                        ...
                                        :class="row.is_revocation ? 'bg-red-500/15 text-red-400 border border-red-500/25' :
                                            'bg-[#1a1a1a] text-[#333] border border-[#222] hover:border-[#333] hover:text-[#555]'">REV</button>
                                </div>
                                <div class="flex justify-center">
                ...
                <div class="flex-shrink-0 px-6 sm:px-8 py-4 border-t border-[#1e1e1e]">
                    <div x-show="bulkSubmitting" x-transition
                        class="h-0.5 bg-[#1e1e1e] mb-4 rounded-full overflow-hidden">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2.5">
                            <button type="button" @click="closeBulkModal()" :disabled="bulkSubmitting"
                                class="px-5 py-2.5 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a] text-[10px] font-bold uppercase tracking-widest hover:bg-[#222] transition disabled:opacity-30 disabled:pointer-events-none">Cancel</button>
        ...
        <div x-show="showLogoutModal" x-cloak
            ...
            <div @click.stop class="glass-panel p-7 sm:p-8 rounded-[2rem] max-w-sm w-full" x-show="showLogoutModal"
                ...
                <div class="text-center">
                    <div
                        class="w-14 h-14 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-5 border border-red-500/20">
                        <i class="fa-solid fa-power-off text-red-400 text-lg"></i></div>
